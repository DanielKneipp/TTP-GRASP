...
void TTPInstance::evaluateSolution( TTPSolution& individual )
{
    double fitness = 0.0;
    ...
    if( !this->isValidSolution( individual ) )
    {
        individual.perf = -std::numeric_limits< double >::infinity();
        return;
    }
...
bool TTPInstance::isValidSolution( TTPSolution& individual )
{
...
void TTPInstance::removeWorstItemsWhileInvalid( TTPSolution& individual )
{
    ...
    while( !this->isValidSolution( individual ) )
    {
